<script>
	...
	import { CheckboxButton } from 'flowbite-svelte';
...
<div class="w-full grid grid-cols-2 gap-2">
	...
	<div class="flex">
		<input type="checkbox" id={`area-${index}`} bind:checked={area.activated} class="peer hidden" />
		<label
			for={`area-${index}`}
		...
		>
			<div class="flex h-full w-full items-center justify-center p-2 text-center">{area.name}</div>
		</label>
		</div>
	{/each}
</div>
